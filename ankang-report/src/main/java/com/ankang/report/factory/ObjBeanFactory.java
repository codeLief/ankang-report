...
public abstract class ObjBeanFactory implements ReportBeanFactory{
	private static final Logger logger = Logger.getLogger(ObjBeanFactory.class);
	
	protected void putBean(Object object){
		if(null == object){
			return;
		}
		...
		try {
			...
			for (Field field : fields) {
				if (!Modifier.isFinal(field.getModifiers()) 
						...
						&& !field.getGenericType().toString().matches("^.+java\\..+$") 
						){
						Class<?> fieldClazz = getClass(field.getGenericType().toString());
						Object cBean = null;
			...
			logger.error("Failed to do so, can not find calss");
			throw new ReportException("Failed to do so, can not find calss");
		}
		logger.info("install bean:"+object);
		putBean(object);
		return object;
	}
	...
	public Object createBean(String clazz){
		
		try {
			return createBean(getClass(clazz));
		} catch (ClassNotFoundException e) {
			logger.error("bean create fail", e);
			throw new ReportException("bean create fail", e);
		}
	}
	protected abstract Object objProxy(Class<?> clazz);
	protected static void throwException(String message, Object...args){
