...
public class ReportRequestHandler extends ReportCabinet implements
	...
	public ReportResponse handler(String serviceAlias, String methodAlias, 
			ReportResolver resolver, HttpServletRequest request) throws Exception {
		...
		Invoker startInvoke = (Invoker)ReportConfig.getValue(ReportConfigItem.START_INVOKE.getConfigName());
		
		return startInvoke.invoke(request, invocation);
	}

	// 最先从HTTPParam
	// 获取参数名称，如果为空则从spring的RequestParam里面获取
	// 暂不支持map，数组封装
	@Override
	public ReportResponse invoke(HttpServletRequest request,
		...
		if(null == invocation.getArguments() 
				|| invocation.getParameterTypes().length != invocation.getArguments().length){
			
			Class<?>[] parameterTypes = invocation.getParameterTypes();
			Object[] invokerParam = null;
			if (parameterTypes.length > 0) {

				invokerParam = new Object[parameterTypes.length];

				invocation.getResolver().resolverParamter(request);

				for (int i = 0; i < parameterTypes.length; i++) {

					Annotation[][] parameters = invocation.getMethod().getParameterAnnotations();
					...
					if (HttpServletRequest.class
							.isAssignableFrom(parameterTypes[i])) {// 支持request封装
						invokerParam[i] = request;
						continue;
					}else if ((ReportRequest.class.isAssignableFrom(parameterTypes[i]))
						...
						invokerParam[i] = invocation.getResolver().getValue(para.toString(),
								parameterTypes[i]);
					}
				}
			}
			invocation.setArguments(invokerParam);
		}
		
		Object result = invocation.getMethod().invoke(getBean(invocation.getModul()), invocation.getArguments());
